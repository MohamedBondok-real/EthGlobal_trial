...
export default function PrivyAuthButton({ onLoginSuccess }: PrivyAuthButtonProps) {
  ...
  if (isLoggedIn) {
    return (
      <div className="flex items-center gap-2 max-w-full">
        <div className="flex items-center gap-2 px-3 py-1.5 rounded-xl bg-slate-100 border border-slate-200 text-xs text-slate-800 shadow-sm min-w-0 max-w-[180px] sm:max-w-[220px]">
          <User className="w-3.5 h-3.5 text-orange-600 shrink-0" />
          <span className="font-mono text-slate-700 truncate">{userEmail}</span>
        </div>
        <button
  ...
  return (
    <>
      ...
      >
        <KeyRound className="w-4 h-4 text-amber-100 shrink-0" />
        <span>Privy Social Login</span>
      </button>

      {showModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm animate-fade-in">
          <div className="w-full max-w-md rounded-2xl bg-white border border-slate-200 p-6 shadow-2xl relative overflow-hidden">
            ...
            <form onSubmit={handleSimulatedLogin} className="space-y-4">
              <div>
                ...
                <input
                  ...
                  className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-2.5 text-xs text-slate-900 placeholder-slate-400 focus:outline-none focus:border-pink-500 focus:ring-1 focus:ring-pink-500 transition-all"
                />
              </div>
            ...
            <div className="mt-4 pt-3 border-t border-slate-100 text-[10px] text-center text-slate-500">
